...
function Booking() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (alertedWorkers === 0) {
        ...
        toast.success('Your booking request was sent to this professional.');
      } else {
  ...
  if (isLoading && workerId) {
    return <div className="container" style={{ textAlign: 'center', padding: '4rem', fontSize: '1.25rem', color: 'var(--text-light)' }}>Loading worker details...</div>;
  }
  ...
  return (
    <div className="container" style={{ maxWidth: '700px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2.5rem', padding: '0 2rem' }}>
        ...
        <div style={{ flex: 1, height: '4px', background: step >= 2 ? 'var(--primary)' : 'var(--border)', margin: '18px 1rem 0 1rem', borderRadius: '2px' }} />
        ...
        <div style={{ flex: 1, height: '4px', background: step >= 3 ? 'var(--primary)' : 'var(--border)', margin: '18px 1rem 0 1rem', borderRadius: '2px' }} />
          ...
          <div style={{ animation: 'fadeIn 0.3s ease-out' }}>
            ...
            {form.priority === 'Emergency' && (
              <p style={{ fontSize: '0.85rem', color: 'var(--danger)', fontWeight: '700', marginTop: '0.75rem' }}>
                Emergency jobs include priority handling and a fixed Rs. 600 surcharge.
              </p>
            )}

            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '2.5rem' }}>
              <button disabled={!form.start_time} className="btn" onClick={() => setStep(2)} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>Confirm Time <FaArrowRight /></button>
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div style={{ animation: 'fadeIn 0.3s ease-out' }}>
            ...
            <div className="form-group">
              ...
              <textarea rows="5" placeholder="Provide a brief summary of the exact service you need..." value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} style={{ padding: '1rem', fontSize: '1rem', border: '1px solid var(--border)', borderRadius: '4px' }} />
            </div>
        ...
        {step === 3 && (
          <div style={{ animation: 'fadeIn 0.3s ease-out', textAlign: 'center' }}>
            ...
            <div style={{ background: '#f8fafc', padding: '1.5rem', borderRadius: '8px', textAlign: 'left', marginBottom: '2.5rem', border: '1px solid var(--border)' }}>
              ...
              <p style={{ margin: '0 0 0.75rem 0' }}><strong style={{ color: 'var(--text-light)', minWidth: '130px', display: 'inline-block' }}>Est. Total Cost:</strong> <strong style={{ color: 'var(--primary)', fontSize: '1.2rem' }}>Rs. {calculateTotal()}</strong></p>
